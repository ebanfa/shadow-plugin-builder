...
<#list entity.fields as field>
    ...
    public static function init_ajax_hooks() {
        ...
        add_action('wp_ajax_publish_dispute_ajax', '${entity.name}API::publish_dispute_ajax');
        add_action('wp_ajax_nopriv_publish_dispute_ajax', '${entity.name}API::publish_dispute_ajax');
    }
...
<#list entity.fields as field>
    ...
    *
    */
   public static function publish_dispute_ajax(){
        // Ensure we have a valid form
        if (!isset($_POST['submitted']) && !isset($_POST['post_nonce_field']) && !wp_verify_nonce($_POST['post_nonce_field'], 'post_nonce')) {
            // Nounce field did not validate
            wp_send_json_error(array('message' => "<span class='error'>Invalid form operation!</span>"));
        ...
        if (!isset($_POST['id']) ) {
            // Nounce field did not validate
            wp_send_json_error(array('message' => "<span class='error'>Dispute identifier missing</span>"));
        }
        $id = sanitize_text_field($_POST['id']);
        ...
        if(isset($post['id'])){
           ...
           update_post_meta($post['id'], 'status', $status['id']);
            ...
            wp_send_json_error(array('message' => "<span class='error'>Invalid dispute identifier</span>"));
        }
