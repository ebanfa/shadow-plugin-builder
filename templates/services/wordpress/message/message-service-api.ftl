...
<#list entity.fields as field>
    ...
    public static function get_by_conversation_id($conversation_id){
        $messages = array();
        $messageQueryArgs = array('numberposts' => -1, 'post_status' => 'publish', 'post_type' => '${entity.postName}',
        'meta_query' => array(array('key' => 'conversation', 'value' => $conversation_id)));
        $messageQuery = new WP_Query($messageQueryArgs);
        while ($messageQuery->have_posts()) : $messageQuery->the_post();
            $entity = $messageQuery->post;
            $message = ${entity.name}API::entity_to_data($entity, false);
