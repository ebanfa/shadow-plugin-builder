...
<#list entity.fields as field>
	...
	$entity_data['edit_mode'] = true;
        ...
        $level = NotificationLevelAPI::get_by_code($notification_data['log_level']); 
        if(isset($level['id'])){ $entity_data['log_level'] = $level['id'];}
        
        $type = NotificationTypeAPI::get_by_code($notification_data['type']); 
        if(isset($type['id'])){ 
            $entity_data['type'] = $type['id'];
            $entity_data['name'] = ${entity.name}API::fill_template($entity_data, $type['title_template']);
            $entity_data['description'] = ${entity.name}API::fill_template($entity_data, $type['message_template']);
        } 
    ...
    public static function fill_template($entity_data, $template){
        foreach ($entity_data as $key => $value) {
            if($key == 'owner') {
                $user_party = PartyAPI::get_by_id($value);
    ...
    public static function get_notification_items($party_id){
        ...
        $entities = array();
        $status = ${entity.name}API::get_status_by_code('PENDING');
        $entityQueryArgs = array('numberposts' => -1, 'post_status' => 'publish', 'post_type' => '${entity.postName}',
            'meta_query' => array(array('key' => 'owner', 'value' => $party_id), array('key' => 'status', 'value' => $status['id'])));
        $entityQuery = new WP_Query($entityQueryArgs);
        while ($entityQuery->have_posts()) : $entityQuery->the_post();
            $entity = $entityQuery->post;
            array_push($entities, ${entity.name}API::entity_to_data($entity, false));
        endwhile;
        return $entities;
    }
