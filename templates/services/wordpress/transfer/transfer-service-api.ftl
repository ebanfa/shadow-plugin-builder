...
<#list entity.fields as field>
     ...
     *
     */
    public static function transfer_funds($amount, $source_user, $destination_user, $description){
        $transfer_results = array('has_errors' => false);
        // Verify that the amount is greater than zero
        if($amount == 0) {
            $transfer_results['has_errors'] = true;
            $transfer_results['error_message'] = 'Invalid bid amount';
	    return $transfer_results;
        }

        // Find the source account 
        $source_account = ${entity.name}API::get_user_account($source_user);
        if(!isset($source_account['id'])) {
            $transfer_results['has_errors'] = true;
            $transfer_results['error_message'] = 'Cannot find source account';
	    return $transfer_results;
        }

        // Find the destination account 
        $count = 0;
        $destination_account = ${entity.name}API::get_user_account($destination_user);
        if(!isset($destination_account['id'])) {
            $transfer_results['has_errors'] = true;
            $transfer_results['error_message'] = 'Cannot find destination account';
	    return $transfer_results;
        }

        // Verify is the source account has enough funds
        if($source_account['balance'] < $amount){
            $transfer_results['has_errors'] = true;
            $transfer_results['error_message'] = 'Insufficient funds available in source account';
	    return $transfer_results;
        }
        // Deduct the amount from the source and add the amount to the target
        $new_source_balance = $source_account['balance'] - $amount;
        update_post_meta($source_account['id'], 'balance', $new_source_balance);
        
        $new_destination_balance = $destination_account['balance'] + $amount;
        update_post_meta($destination_account['id'], 'balance', $new_destination_balance);
        
        // Create an new transfer object to capture the above.
        $transfer_data = array();
        $transfer_data['edit_mode'] = true;
        $transfer_data['description'] = $description; 
        $transfer_data['owner'] = $source_user['id'];
        $transfer_data['name'] = $source_user['id'];
        $transfer_data['destination_user'] = $destination_user['id'];
        $transfer_data['amount'] = $amount;
        $transfer_data['payment_date'] = date("Y-m-d H:i:s");
     
        $status = ${entity.name}API::get_status_by_code('COMPLETED');
        $transfer_data['status'] = $status['id'];
        ${entity.name}API::do_create_entity($transfer_data);

        // Create a new transaction object to capture the above.
        $transaction_data = array();
        $transaction_data['edit_mode'] = true;
        $transaction_data['owner'] = $source_user['id']; 
        $transaction_data['date_created'] =  date("Y-m-d H:i:s");
        $transaction_data['date_posted'] =  date("Y-m-d H:i:s");
        $transaction_data['amount'] = $amount;
        $transaction_data['name'] = $source_user['id'];
        $transaction_data['description'] = $description;
        $status = TransactionAPI::get_status_by_code('COMPLETED');
        $transaction_data['status'] = $status['id'];

        $transaction_type = TransactionTypeAPI::get_by_code('TRANSFER');
        $transaction_data['type'] = $transaction_type['id'];
        // Save the transaction
        TransactionAPI::do_create_entity($transaction_data);
    }
    
    public static function get_user_account($user){
        $count = 0;
        $account_data = array();
 
        // Get the party account of the party
        $count = 0;
        $queryArgs = array('numberposts' => -1, 'posts_per_page' => -1,
            'post_status' => 'any', 'post_type' => 'sb_partyaccount', 
            'meta_query' => array(array('key' => 'owner', 'value' => $user['id'])));
        
        $accounts_query = new WP_Query($queryArgs);
        while ($accounts_query->have_posts()) : $accounts_query->the_post();
            $account = $accounts_query->post;
            if($count == 0 ) {
               $account_data = PartyAccountAPI::entity_to_data($account, true);
            }
            $count++;
        endwhile;
        wp_reset_postdata();
