...
<#list entity.fields as field>
    ...
    public static function init_ajax_hooks() {
        ...
        add_action('wp_ajax_publish_application_ajax', '${entity.name}API::publish_application_ajax');
        add_action('wp_ajax_nopriv_publish_application_ajax', '${entity.name}API::publish_application_ajax');
    }
...
<#if entity.global == "N"> 
        ...
        if ($entity_data['edit_mode']) {
            ...
            if(!is_wp_error($entity_data['id'])){
                ...
                $notification_data['type'] = 'APPL_CREATED';
                $notification_data['owner'] = $entity_data['owner'];
                $notification_data['log_level'] = 'INFO';
               
		// Do notification
		do_action('shadowbanker_notify_user', $notification_data); 
            }
...
<#list entity.fields as field>
    ...
    <#if field.relationshipField == "Y">
        ...
        $entity_data['owner_name'] = $user_party['name'];
        <#else>
        $entity_data['${field.name}'] = $related_entity_id;
     ...
     *
     */
    public static function can_bid($id){
       if(!$id)
           return false;
       ...
       $status = ApplicationStatusAPI::get_by_id($status_id); 
       if($status['entity_code'] != 'PUBLISHED')
           return false;
        
       // Check the owner is not trying to bid on his/her own loan
       $user_party = PartyAPI::get_by_id($post['owner']);
       $party_user = PartyAPI::get_party_user($user_party['id']);

       $current_user = wp_get_current_user();
       if($current_user->ID == $party_user['id'])
            return false;
      
       // Lastly check if the user has any previous bids for this application
       $party_of_current_user = PartyAPI::get_user_party($current_user->ID);
       $bidQueryArgs = array('numberposts' => -1, 'post_status' => 'any', 'post_type' => 'sb_bid',
            'meta_query' => array(array('key' => 'application', 'value' => $id),
                                  array('key' => 'owner', 'value' => $party_of_current_user['id'])));
       $bidQuery = new WP_Query($bidQueryArgs);
       $can_bid = true;
       while ($bidQuery->have_posts()) : $bidQuery->the_post();
           $bid = $bidQuery->post;
           $can_bid = false;
       endwhile;
   ...
   public static function publish_application_ajax(){
        ...
        if (!isset($_POST['id']) ) {
            // Nounce field did not validate
            wp_send_json_error(array('message' => "<span class='error'>Application identifier missing</span>"));
        }
        $id = sanitize_text_field($_POST['id']);
        $post = ${entity.name}API::get_by_id($id);
        if(isset($post['id'])){
           ...
           update_post_meta($post['id'], 'status', $status['id']);
            ...
            wp_send_json_error(array('message' => "<span class='error'>Invalid application identifier</span>"));
        }
