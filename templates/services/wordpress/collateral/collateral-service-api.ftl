...
class CollateralAPI {
    ...
    public static function init_ajax_hooks() {
        add_action('wp_ajax_create_sb_collateral_ajax', 'CollateralAPI::create_sb_collateral_ajax');
        add_action('wp_ajax_nopriv_create_sb_collateral_ajax', 'CollateralAPI::create_sb_collateral_ajax');

        add_action('wp_ajax_edit_sb_collateral_ajax', 'CollateralAPI::edit_sb_collateral_ajax');
        add_action('wp_ajax_nopriv_edit_sb_collateral_ajax', 'CollateralAPI::edit_sb_collateral_ajax');

        add_action('wp_ajax_view_sb_collateral_ajax', 'CollateralAPI::view_sb_collateral_ajax');
        add_action('wp_ajax_nopriv_view_sb_collateral_ajax', 'CollateralAPI::view_sb_collateral_ajax');

        add_action('wp_ajax_find_sb_collateral_ajax', 'CollateralAPI::find_sb_collateral_ajax');
        add_action('wp_ajax_nopriv_find_sb_collateral_ajax', 'CollateralAPI::find_sb_collateral_ajax');

        add_action('wp_ajax_delete_sb_collateral_ajax', 'CollateralAPI::delete_sb_collateral_ajax');
        add_action('wp_ajax_nopriv_delete_sb_collateral_ajax', 'CollateralAPI::delete_sb_collateral_ajax');
    }
    
    /**
     *
     */
    public static function create_sb_collateral_ajax() {
        ...
        $entity_data = CollateralAPI::build_entity_data_from_post();
        // Validate the posted data
        $entity_data = CollateralAPI::validate_entity_data($entity_data);
        ...
        $entity_data = CollateralAPI::do_create_entity($entity_data);
        ...
        if(!$entity_data['has_errors']) {
            ...
            if ($entity_data['requires_redirect']) {
                ...
                $redirect_url = get_site_url() . '/page?type=entity&artifact=collateral&id=' . $entity_data['id'] . '&page_action=view';
            }
    ...
    public static function build_entity_data_from_post(){
        ...
        if($entity_data['edit_mode']){
            if (isset($_POST['type']))
                $entity_data['type'] = sanitize_text_field($_POST['type']);
            if (isset($_POST['name']))
                $entity_data['name'] = sanitize_text_field($_POST['name']);
            if (isset($_POST['market_value']))
                $entity_data['market_value'] = sanitize_text_field($_POST['market_value']);

            if(isset($_POST['application'])){
                $entity_data['application'] = $_POST['application'];
            }
            if (isset($_POST['cost']))
                $entity_data['cost'] = sanitize_text_field($_POST['cost']);
            if(CloderiaServiceUtils::is_invalid_string($_POST['date_acquired'])) {
                $entity_data['date_acquired'] = date("Y-m-d H:i:s");
            }
            else{
               $entity_data['date_acquired'] = sanitize_text_field($_POST['date_acquired']);
            }
    ...
    public static function validate_entity_data($entity_data) {
           ...
           if($entity_data['edit_mode']) {
            if(empty($entity_data['type'])){
            	$entity_data['has_errors'] = true; 
                array_push($entity_data['error_fields'], 'type');
            }
            if(empty($entity_data['name'])){
            	$entity_data['has_errors'] = true; 
                array_push($entity_data['error_fields'], 'name');
            }
            if(empty($entity_data['market_value'])){
            	$entity_data['has_errors'] = true; 
                array_push($entity_data['error_fields'], 'market_value');
            }
            if(empty($entity_data['cost'])){
            	$entity_data['has_errors'] = true; 
                array_push($entity_data['error_fields'], 'cost');
            }
            if(empty($entity_data['date_acquired'])){
            	$entity_data['has_errors'] = true; 
                array_push($entity_data['error_fields'], 'date_acquired');
            }
    ...
    public static function do_create_entity($entity_data){
        ...
        if ($entity_data['edit_mode']) {
            ...
            $status = CollateralAPI::get_status_by_code('PENDING');
            $entity_data['status'] = $status['id'];
            
            CollateralAPI::copy_fields_to_post($entity_data);
            // Post information
            $post_information = array('post_title' => $entity_data['name'], 
                'post_content' => esc_attr($entity_data['name']), 
                'post_type' => 'sb_collateral', 'post_status' => 'publish');
     ...
     *
     */
    public static function find_sb_collateral_ajax() {
        ...
        $queryArgs = array('numberposts' => -1, 'posts_per_page' => -1,
            'post_status' => 'any', 'post_type' => 'sb_collateral', 'meta_query' => $meta_array);
        $count = 0;
        $searchResults = array();
        ...
        while ($entityQuery->have_posts()) : $entityQuery->the_post();
            ...
            array_push($searchResults, CollateralAPI::entity_to_data($entity, false));
            $count++;
        endwhile;
        wp_reset_postdata();
     ...
     *
     */
    public static function delete_sb_collateral_ajax() {
        // Ensure we have a valid form
        if (!isset($_POST['submitted']) && !isset($_POST['post_nonce_field']) && !wp_verify_nonce($_POST['post_nonce_field'], 'post_nonce')) {
            // Nounce field did not validate
            wp_send_json_error(array('message' => "<span class='error'>Invalid form operation!</span>"));
        ...
        // Process the results of the order creation
        if ($post_obj) {
            $redirect_url = get_site_url() . '/page?type=entity&artifact=collateral&page_action=list';
            wp_send_json_success(array('message' => "<script type='text/javascript'>window.location='" . $redirect_url . "'</script>"));
        } else {
    ...
    public static function get_by_code($entity_code){
        // Load the entity
        $entity_data = array();
        $entityQueryArgs = array('numberposts' => -1, 'post_status' => 'any', 'post_type' => 'sb_collateral',
            'meta_query' => array(array('key' => 'entity_code', 'value' => $entity_code)));
        $entityQuery = new WP_Query($entityQueryArgs);
        while ($entityQuery->have_posts()) : $entityQuery->the_post();
            $entity = $entityQuery->post;
            $entity_data = ${entity.name}API::entiity_to_data($entity);
        endwhile;
        return $entity_data;
    }
    ...
    public static function get_by_id($id){
        ...
        return CollateralAPI::entity_to_data($post_obj, false);
    }
