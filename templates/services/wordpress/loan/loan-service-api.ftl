...
<#list entity.fields as field>
    ...
    public static function init_ajax_hooks() {
        ...
        add_action('wp_ajax_repay_loan_ajax', '${entity.name}API::repay_loan_ajax');
        add_action('wp_ajax_nopriv_repay_loan_ajax', '${entity.name}API::repay_loan_ajax');
    }
...
<#if entity.global == "N"> 
    ...
    public static function find_${entity.postName}_ajax() {
        ...
        $owner_query_param = array('key' => 'owner', 'value' => $user_party['id']);
        $client_query_param = array('key' => 'counter_party', 'value' => $user_party['id']);
        array_push($queryArgs['meta_query'], $owner_query_param);
        array_push($queryArgs['meta_query'], $client_query_param);
...
<#list entity.fields as field>
    ...
    <#if field.relationshipField == "Y">
        <#if field.name == "owner">
            $owner_id = get_post_meta($entity->ID, 'owner', true);
            $owner_party = PartyAPI::get_by_id($owner_id);
            $entity_data['${field.name}'] = $owner_party['id'];

            // Get the related post
            $entity_data['${field.name}_txt'] = $owner_party['user_name'];
            $entity_data['${field.name}_code'] = $owner_party['user_name'];

        <#elseif field.name =="counter_party">
            $client_party_id = get_post_meta($entity->ID, 'owner', true);
            $client_party = PartyAPI::get_by_id($client_party_id);
            $entity_data['${field.name}'] = $client_party['id'];
            // Get the related post
            $entity_data['${field.name}_txt'] = $client_party['user_name'];
            $entity_data['${field.name}_code'] = $client_party['user_name'];
     ...
     *
     */
    public static function create_shadow_loan($bid_data){
        $loan_owner = $bid_data['owner'];
	$application = ApplicationAPI::get_by_id($bid_data['application']);
              
        $loan_data = array();   
        $loan_data['edit_mode'] = true;
        $loan_data['name'] = $application['name'];
        $loan_data['owner'] = $loan_owner; 
        $loan_data['counter_party'] = $application['owner'];
        $loan_data['amount'] = $bid_data['amount'];
        $loan_data['balance'] = $bid_data['amount'];
        $loan_data['rate'] = $bid_data['rate'];
        $loan_data['term'] = $bid_data['term'];
        $loan_data['application'] = $application['id'];
        
        $status = ${entity.name}API::get_status_by_code('ACTIVE');
        $loan_data['status'] = $status['id'];
        $loan_data['date_created'] = date("Y-m-d H:i:s");

        //
        ${entity.name}API::do_create_entity($loan_data);
   }

   /**
    *
    */
   public static function repay_loan_ajax(){
        // Ensure we have a valid form
        if (!isset($_POST['submitted']) && !isset($_POST['post_nonce_field']) && !wp_verify_nonce($_POST['post_nonce_field'], 'post_nonce')) {
            // Nounce field did not validate
        ...
        if (!isset($_POST['id']) ) {
            // Nounce field did not validate
            wp_send_json_error(array('error_type' => 'data_error', 'message' => "Loan identifier missing"));
        }
        $id = sanitize_text_field($_POST['id']);
        $post = ${entity.name}API::get_by_id($id);
        if(isset($post['id'])){
            // The source user is the owner of the application. We are transferring funds from the account of
            // the account of the source user to the holding account.
            $application = ApplicationAPI::get_by_id($post['application']);
            $source_user_party = PartyAPI::get_by_id($application['owner']);
            if(!isset($source_user_party['id'])) {
                wp_send_json_error(array('error_type' => 'business_error', 'message' => "Cannot find funds source user!"));
            }
            ...
            $transfer_desc = 'Repayment of loan funds to client '. $source_user_party['user_name'];
            $transfer_results = TransferAPI::transfer_funds($amount, $source_user_party, $destination_user_party, $transfer_desc);      
            ...
            $status = ${entity.name}API::get_status_by_code('REPAID');
            ...
            wp_send_json_error(array('error_type' => 'business_error', 'message' => "Invalid loan identifier!"));
        }
   }
    ...
    *
    */
   public static function can_repay_loan($loan_id){
       $current_user = wp_get_current_user();
       $loan = ${entity.name}API::get_by_id($loan_id);

       $application = ApplicationAPI::get_by_id($loan['application']);
       if(!isset($application['owner'])) {
           return false;
       }
        
       $current_user_party = PartyAPI::get_user_party($current_user->ID);
       if($current_user_party['id'] != $application['owner']){
           return false;
       }
       // Check the loan status
       $active_status = ${entity.name}API::get_status_by_code('ACTIVE');
       if($active_status['id'] != $loan['id']){ return false;}
       return true;
   }
}
