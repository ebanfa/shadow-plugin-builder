...
class PartyAPI {
    ...
    public static function do_create_entity($entity_data){
        ...
        if(isset($entity_data['user_data']) && isset($entity_data['id']))
            CloderiaUserAPI::send_user_created_email($entity_data['user_data']);
    ...
    public static function do_create_individual($entity_data){
        ...
        if(isset($entity_data['user_data']))
            // This is required when sending emails for account creation notification
            $party_data['user_data'] = $entity_data['user_data'];
     ...
     */
    public static function add_role_to_party($party_data, $cust_role_type_code) {
        $entity_data = EntityAPIUtils::init_entity_data('partyrole');
        $owner_role_data = EntityAPI::get_by_code('roletype', strtoupper($cust_role_type_code));
    ...
    public static function is_portal_admin($party_data){
        if($party_data['entity_code'] == 'ADMINISTRATOR')
            return true;
        return false;
    }
