...
function cp_get_template_part($slug, $name = '') {
    ...
    if ($name) {
        $template = locate_template(array("{$slug}-{$name}.php", ${application.name}::template_path() . "{$slug}-{$name}.php"));
    }
    // Get default slug-name.php
    if (!$template && $name && file_exists(${application.name}::plugin_path() . "/templates/{$slug}-{$name}.php")) {
        $template = ${application.name}::plugin_path() . "/templates/{$slug}-{$name}.php";
    }
    // If template file doesn't exist, look in yourtheme/slug.php and yourtheme/woocommerce/slug.php
    if (!$template) {
        $template = locate_template(array("{$slug}.php", ${application.name}::template_path() . "{$slug}.php"));
    }
...
function cp_locate_template($template_name, $template_path = '', $default_path = '') {
    if (!$template_path) {
        $template_path = ${application.name}::template_path();
    }
    if (!$default_path) {
