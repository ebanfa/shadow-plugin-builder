...
class EntityActionProcessor {
    ...
    public static function init_hooks() {
        ...
        add_action('wp_ajax_quick_create_product_ajax', 'EntityActionProcessor::quick_create_product_ajax');
        add_action('wp_ajax_nopriv_quick_create_product_ajax', 'EntityActionProcessor::quick_create_product_ajax');
    ...
    public static function confirm_product_order_ajax() {
        ...
        $entity_data = ProductOrderAPI::do_create_user_order($user_data);
     ...
     *
     */
    public static function quick_create_product_ajax() {
        // Ensure we have a valid form
        if(!EntityRequestUtils::is_valid_form() || !isset($_POST['edit_mode']) || !isset($_POST['id']) || !isset($_POST['product_name']) || !isset($_POST['inventory_count']) || empty($_FILES))
            wp_send_json_error(array('message' => "Invalid artifact operation!"));

        var_dump($_POST);

        $entity_data = EntityAPIUtils::init_entity_data('product');
        $entity_data['images_param'] = 'product_images';
        $entity_data['template_id'] = sanitize_text_field($_POST['id']);
        $entity_data['name'] = sanitize_text_field($_POST['product_name']);
        $entity_data['inventory_count'] = sanitize_text_field($_POST['inventory_count']);
        $entity_data = ProductTemplateAPI::do_create_product_entity($entity_data);
        self::do_after_ajax_edit($entity_data);
    }
}
