...
?>
    ...
    <#list entity.fields as field>
            ...
            <#if field.relationshipField == "Y">
                <td><?php echo $entity_data['${field.name}_txt']; ?></td>
 				   ...
 				   <?php if($is_pending){ ?>
                                    ...
                                    <form id="publish-dispute-form" style="display:none" action=""  method="POST">
                                        <input type="hidden" name="id" value="<?php echo $entity_data['id']; ?>">
                                        <?php wp_nonce_field('post_nonce', 'post_nonce_field'); ?>
                                        <input type="hidden" name="submitted" id="submitted" value="true" />
                                    </form>
                                    <a id="publish-dispute-btn" href="<?php echo get_site_url() . '/page?type=entity&artifact=' . $entity_name . '&id=' . $entity_data['id']; ?>&page_action=delete" class="btn btn-success waves-effect">
                                       <?php _e('Publish', 'framework') ?>
                                    </a>
                                    <?php } ?>
                                    <a href="<?php echo get_site_url() . '/page?type=entity&artifact=loan&id=' . $entity_data['loan']; ?>&page_action=view" 
                                       class="btn btn-primary waves-effect">
        ...
        <form id="sb_disputeitem-list-form">
            <input type="hidden" name="application" value="<?php echo $entity_data['id']; ?>"/>
            <?php wp_nonce_field('post_nonce', 'post_nonce_field'); ?>
            <input type="hidden" name="submitted" id="submitted" value="true" /> 
        </form>
        <div class="table-responsive">
            <table id="sb_disputeitem-table" class="table table-striped table-bordered table-hover" width="100%" cellspacing="0">
                <thead>
                    <tr>
                        <th>ID</th>
            		<th>Dispute</th>
               ...
               <!--  <tfoot>
                ...
                <tbody>
                                ...
                                <div class="btn-demo m-t-10">
                                    ...
                                    <a id="create-disputeitem-btn" href="<?php echo get_site_url();?>/page?type=entity&artifact=disputeitem&page_action=create&dispute_id=<?php echo $entity_data['id']; ?>" class="btn btn-success waves-effect">
                                       <?php _e('Add Item', 'framework') ?>
                                    </a>
                                </div>
...
<script type="text/javascript">
        ...
        //Publish Message
        $('#publish-dispute-btn').click(function(e){
            e.preventDefault();
            swal({   
                title: "Great job!",   
                text: "Please wait a moment while we publish your dispute...",   
                type: "success",   
                showCancelButton: false, 
                showConfirmButton: false ,
            });
            var form = $('#publish-dispute-form').ajaxSubmit(
            { /* options */ 
                url: ${application.name?lower_case}_ajax_script.ajaxurl,
                data: ({action : 'publish_dispute_ajax'}),  
