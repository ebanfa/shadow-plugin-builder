<?php
    ...
    if (isset($_REQUEST['id'])) {
        $entity_data = PaymentAPI::find_by_id(sanitize_text_field($_REQUEST['id']));
        $is_pending = false;
...
?>
                                ...
                                <div class="body-section">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-striped table-bordered table-hover" width="100%" cellspacing="0">
                                            <tbody>
                                                <th><h5 class="no-margin-bottom">Recipient</h5></th>
                ...
                <td><?php echo $entity_data['payment_date']; ?></td>
...
?>
                <div class="btn-demo m-t-10">
                    <?php if($is_pending) { ?>
                    <form id="send-funds-form" style="display:none" action=""  method="POST">
                        <input type="hidden" name="id" value="<?php echo $entity_data['id']; ?>">
                        <?php wp_nonce_field('post_nonce', 'post_nonce_field'); ?>
                        <input type="hidden" name="submitted" id="submitted" value="true" />
                    </form>
                    <a id="send-funds-btn" href="<?php echo get_site_url() . '/page?type=entity&artifact=' . $entity_name . '&id=' . $entity_data['id']; ?>&page_action=delete" class="btn btn-success waves-effect">
                       <?php _e('Send Payment', 'framework') ?>
                    </a>
...
<script type="text/javascript">
        ...
        $('#delete-entity-btn').click(function(e){
            ...
            swal({   
                ...
                {/* options */
                    ...
                    data: ({action: 'delete_sb_payment_ajax'}),
        ...
        //Publish Message
        $('#send-funds-btn').click(function(e){
            e.preventDefault();
            var form = $('#send-funds-form').ajaxSubmit(
            { /* options */ 
                url: ${application.name?lower_case}_ajax_script.ajaxurl,
                data: ({action : 'send_funds_ajax'}),  
                success: function(responseData)
                {
                    if(responseData.success) {
                        ...
                        swal({   
                            title: "Funds sent!",   
                            text: "The funds have been successfully sent ",   
                            type: "success",   
                            showCancelButton: false,  
                            confirmButtonText: "OK",   
                            closeOnConfirm: true 
