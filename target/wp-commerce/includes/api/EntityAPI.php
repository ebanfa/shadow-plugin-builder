...
class EntityAPI {
    ...
    public static function process_field_criteria($entity_data) {
        ...
        CloderiaLogUtils::shadow_log('Processing criteria search for field with nick name: ' . $field_criteria_data['criteria_name']);
        ...
        foreach ($entity_data['entity_fields'] as $field_data) {
            ...
            if($field_data['nick_name'] == $field_criteria_data['criteria_name']) {
                ...
                CloderiaLogUtils::shadow_log('Resolved criteria search field name to: ' . $field_criteria_data['criteria_name']);
                // Are we dealing with a relationship field
                if($field_data['is_relationship_field']) {
                    CloderiaLogUtils::shadow_log('Criteria field is a relationship field to ' . $field_data['entity_name']);
                    ...
                    if(!is_numeric($field_criteria_data['criteria_value'])) {
                        CloderiaLogUtils::shadow_log('Criteria field is non numeric');
                        ...
                        if(isset($parent_entity_data['id'])){
                            CloderiaLogUtils::shadow_log('Found parent instance: ' . $parent_entity_data['entity_code']);
        ...
        CloderiaLogUtils::shadow_log('Returning criteria data');
