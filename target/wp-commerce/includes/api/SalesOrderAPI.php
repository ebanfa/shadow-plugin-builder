...
class SalesOrderAPI {
    ...
    public static function do_create_sales_order($user_data, $shopping_cart_data){
        ...
        // Check for errors
        if($user_party['has_errors']) return EntityAPIUtils::init_error($order_entity_data, $user_party['message']); 
        // Process the order
        if(isset($business_party['id']) && isset($user_party['id'])) {
            ...
            $order_entity_data['edit_mode'] = false;
            ProductOrderAPI::do_create_entity($order_entity_data);
        }
        return $order_entity_data;
    }
