...
class ProductAPI {
    ...
    public static function load_products($product_type_code){
        ...
        if(isset($product_type['id'])) {
            ...
            if(!isset($currency_data['id']))
                return EntityAPIUtils::init_error($product_type, 'Default currency not configured');
            ...
            foreach ($product_data_list as $key => $product_data) {
                if(isset($product_data['id'])) {
                    ...
                    // Currency
                    $product_data['currency'] = $currency_data['symbol'];
                    // Process the pricing of the product
                    $product_price = 0;
                    $price_components = EntityAPI::find_by_criteria(
                        'pricecomponent', array('component_prod' =>  $product_data['id']));
    ...
    public static function load_product_data($product_code){
        ...
        if(isset($product_data['id'])) {
            ...
            $currency_data = BusinessAPI::get_default_currency();
            if(!isset($currency_data['id']))
                return EntityAPIUtils::init_error($product_data, 'Default currency not configured');
            $product_data['currency'] = $currency_data['symbol'];
