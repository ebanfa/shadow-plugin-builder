...
class ProductTemplateAPI {
    ...
    public static function do_create_product_entity($entity_data) {
        ...
        $entity_data['name'] = $entity_data['name'];
        $entity_data['description'] = $entity_data['name'];
        $entity_data['prod_template'] = $template_data['id'];
        // 4. Create product
    ...
    public static function do_create_inventory_item($entity_data, $inventory_count) {
        ...
        $inventory_item_data['quantity'] = $inventory_count;
    ...
    public static function do_create_base_price_component($entity_data, $base_price) {
        $price_component_data = EntityAPIUtils::init_entity_data('pricecomponent');

        $price_component_type = EntityAPI::get_by_code('pricecomponenttype', 'BASE_PRICE');
        if(!isset($price_component_type['id']))
            return EntityAPIUtils::init_error($price_component_data, 'Price component type is required');

        $price_component_data['edit_mode'] = true;
        $price_component_data['name'] = $entity_data['name'];
        $price_component_data['component_price'] = $base_price;
        $price_component_data['description'] = $entity_data['name'];
        $price_component_data['component_prod'] = $entity_data['id'];
        $price_component_data['component_type'] = $price_component_type['id'];
        // 5. Create the entity
        return EntityAPI::create_entity($price_component_data);
    }
}
