...
class ViewUtils {
    ...
    public static function get_list_entity_action_links($page_action, $artifact) {
    	$action_links = array();
        $create_entity_link = EntityActionProcessor::get_base_url() . self::$base_create_path. $artifact;
        if(isset($_REQUEST['criteria_name']) && isset($_REQUEST['criteria_value'])) {
            $create_entity_link = $create_entity_link . '&criteria_name='. sanitize_text_field($_REQUEST['criteria_name']);
            $create_entity_link = $create_entity_link . '&criteria_value='. sanitize_text_field($_REQUEST['criteria_value']);
        }
    	$action_links['create_entity_link'] = array('name' => 'Add New', 'link' => $create_entity_link);
    	return $action_links;
   	}
