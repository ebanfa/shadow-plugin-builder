...
$(document).ready(function (){
   ...
   var sb_producttmpl_rows_selected = [];
   var sb_producttmplTable =  $('#sb_producttmpl-multi-list-table').DataTable({
        "ajax": {
            'type': 'POST',
            'url': wpcommerce_ajax_script.ajaxurl,
            'data': function(d){
               d.action = 'find_entity_ajax';
               d.form = $("#sb_producttmpl-multi-list-form").serializeArray();
            },
        },
        columns: [
            { data: "id" }, 
        ...
        columnDefs: [
            {
                ...
                'className': 'dt-body-center',
                'render': function (data, type, row){
                    return '<input id="producttemplate_' + row.id + '" type="checkbox" value="' + row.id + '" data-dependent-instance-name="' + row.name + '">';
                },
            },
            {
                // The `data` parameter refers to the data for the cell (defined by the
                ...
                "render": function ( data, type, row ) {
                    ...
                    return '<a class="data-table-link" href="' + wpcommerce_base_url.baseUrl + 'artifact=producttemplate&id=' + row.id + '&page_action=view' + parent_params + '" data-related-artifact-name="producttemplate" data-related-instance-name="' + row.name + '" data-related-instance-id="' + row.id + '">' + data +  '</a>';
                },
                "targets": 1
            }
        ],
        ...
        'rowCallback': function(row, data, dataIndex){
         ...
         // If row ID is in the list of selected row IDs
         if($.inArray(rowId, sb_producttmpl_rows_selected) !== -1){
            $(row).find('input[type="checkbox"]').prop('checked', true);
            $(row).addClass('selected');
         }
        }
   ...
   // Handle click on checkbox
   $('#sb_producttmpl-multi-list-table tbody').on('click', 'input[type="checkbox"]', function(e){
      var $row = $(this).closest('tr');
      // Get row data
      var data = sb_producttmplTable.row($row).data();
      // Get row ID
      var rowId = $(this).val();
      // Determine whether row ID is in the list of selected row IDs 
      var index = $.inArray(rowId, sb_producttmpl_rows_selected);
      console.log('This is index:' + index);
      // If checkbox is checked and row ID is not in list of selected row IDs
      if(this.checked && index === -1){
         console.log('This is checked and index:' + index);
         sb_producttmpl_rows_selected.push(rowId);
      // Otherwise, if checkbox is not checked and row ID is in list of selected row IDs
      } else if (!this.checked && index !== -1){
         sb_producttmpl_rows_selected.splice(index, 1);
         console.log('This is not checked and index:' + index);
      }
      if(this.checked){
         $row.css('background-color', 'rgba(255, 152, 0, 0.5)');
      ...
      // Update state of "Select all" control
      updateDataTableSelectAllCtrl(sb_producttmplTable);
      // Prevent click event from propagating to parent
      e.stopPropagation();
   });

   // Handle click on table cells with checkboxes
   $('#sb_producttmpl-multi-list-table').on('click', 'tbody td, thead th:first-child', function(e){
      $(this).parent().find('input[type="checkbox"]').trigger('click');
   });

   // Handle click on "Select all" control
   $('thead input[name="select_all"]', sb_producttmplTable.table().container()).on('click', function(e){
      if(this.checked){
         $('#sb_producttmpl-multi-list-table tbody input[type="checkbox"]:not(:checked)').trigger('click');
      } else {
         $('#sb_producttmpl-multi-list-table tbody input[type="checkbox"]:checked').trigger('click');
      }
      // Prevent click event from propagating to parent
      e.stopPropagation();
   });
   ...
   // Handle table draw event
   sb_producttmplTable.on('draw', function(){
      // Update state of "Select all" control
      updateDataTableSelectAllCtrl(sb_producttmplTable);
   });

   /* Add all check rows in the data table */
   $('body').on('click', '#add-selected-producttemplate-list-btn', function(e){
      e.preventDefault();
      var page_artifact_form = $('#main-entity-post-name').val();
      // Iterate over all selected checkboxes
      var idExists = false;
      $.each(sb_producttmpl_rows_selected, function(index, rowId){

        $.each($('input[name="producttemplate_id[]"]'), function(indexx){ 
            var valueToAdd = $(this).val();
            if(valueToAdd === rowId){
              idExists = true;
            }
        ...
        if(!idExists){
          ...
          // main form 
           $('#' + page_artifact_form).append(
               $('<input>')
                  .attr('type', 'hidden')
                  .attr('name', 'producttemplate_id[]')
                  .val(rowId)
           );
           // Get the value of the name column. Every entity data table has name and description columns
           var dependentInstanceName = $('#producttemplate_' + rowId).data('dependent-instance-name');
           // Add an entry into the visual list of select instances
           $('#producttemplate_dependent_list_box').append($(
                '<div id="producttemplate_list_item_' + rowId + '"> ' + 
                    '<span data-entity-name="producttemplate" ' + 
                        'data-entity-id="' + rowId + '" class="badge producttemplate_dependent_list_item" ' + 
                        'style="cursor: pointer; cursor: hand; background-color: red">X</span>' + dependentInstanceName + 
                '</div>').attr('class', 'list-group-item'));
        }
      });
    ...
    // Handle click on table cells with checkboxes
   $('#producttemplate_dependent_list_box').on('click', '.producttemplate_dependent_list_item', function(e){
      var entityId = $(this).data('entity-id');
      var entityName = $(this).data('entity-name');
      var page_artifact_form = $('#main-entity-post-name').val();
      // first remove the hidden form field and then the list box item
      var existingIds = $('#' + page_artifact_form).find('input[name="producttemplate_id[]"]');
      $.each(existingIds, function(index, rowId){ 
        if($(rowId).val() == entityId) {
          $(rowId).remove();
        }
      ...
      // then remove the list box item
      $('#producttemplate_dependent_list_box').find('#producttemplate_list_item_' + entityId).remove();
