...
class MortgageCPT {
    ...
    public static $post_name = 'sb_mortgage'; 
    ...
    public static $custom_fields =  array(
        ...
        array('name' => 'principal',
            'title' => 'Principal',
            'description' => 'The Principal field',
            'type' => 'text',
        ),
        array('name' => 'rate',
            'title' => 'Interest Rate',
 ...
 public static $related_child_entities = array(
    ...
    {
       register_post_type('sb_mortgage', 
            array(
                'label' => 'Mortgage',
                'labels' => array(
                'add_new'           => 'Add New',
                'add_new_item'      => 'Add New Mortgage',
                'edit_item'         => 'Edit Mortgage',
                'new_item'          => 'New Mortgage',
                'view_item'         => 'View Mortgage',
                'search_items'      => 'Search Mortgage',
                'not_found'         => 'No Mortgage Found ',
                'not_found_in_trash'=> 'Not Found in Trash',
                ),
                'description' => 'Reusable Mortgage',
                'public' => true,
                'show_ui' => true,
                'menu_position' => 5,
  ...
  ------------------------------------------------------------------------------*/
    ...
    public static function sb_mortgage_table_head($defaults){
        ...
        $defaults['rate']  = 'Interest Rate';
        $defaults['description']  = 'Description';
        return $defaults;
    }

    public static function sb_mortgage_table_content($column_name, $post_id){
        if ($column_name == 'entity_code') {
            $field_value = get_post_meta($post_id, 'entity_code', true );
            echo $field_value;
        }
        if ($column_name == 'm_type') {
        ...
        if ($column_name == 'lender') {
            $field_value = get_post_meta($post_id, 'lender', true );
            echo $field_value;
        }
        if ($column_name == 'term') {
        ...
        if ($column_name == 'principal') {
            $field_value = get_post_meta($post_id, 'principal', true );
            echo $field_value;
        }
        if ($column_name == 'rate') {
