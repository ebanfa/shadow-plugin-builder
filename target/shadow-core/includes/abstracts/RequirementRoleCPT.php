...
class RequirementRoleCPT {

    public static $prefix = ''; 

    public static $post_name = 'sb_requirerole'; 
    ...
    public static $custom_fields =  array(
        ...
        array('name' => 'requirement',
            'title' => 'Requirement',
            'description' => 'The Requirement field',
            'type' => 'text',
        ),
        array('name' => 'party_role',
            'title' => 'Party Role',
 ...
 public static $related_child_entities = array(
    ...
    {
       register_post_type('sb_requirerole', 
            array(
                'label' => 'Require Role',
                'labels' => array(
                'add_new'           => 'Add New',
                'add_new_item'      => 'Add New Require Role',
                'edit_item'         => 'Edit Require Role',
                'new_item'          => 'New Require Role',
                'view_item'         => 'View Require Role',
                'search_items'      => 'Search Require Role',
                'not_found'         => 'No Require Role Found ',
                'not_found_in_trash'=> 'Not Found in Trash',
                ),
                'description' => 'Reusable Require Role',
                'public' => true,
                'show_ui' => true,
                'menu_position' => 5,
  ...
  ------------------------------------------------------------------------------*/
    ...
    {
        if ( $post->post_type == 'sb_requirerole') 
        {
    ...
    public static function sb_requirerole_table_head($defaults){
        $defaults['requirement']  = 'Requirement';
        $defaults['party_role']  = 'Party Role';
        $defaults['name']  = 'name';
    ...
    public static function sb_requirerole_table_content($column_name, $post_id){
        if ($column_name == 'entity_code') {
            $field_value = get_post_meta($post_id, 'entity_code', true );
            echo $field_value;
        }
        if ($column_name == 'requirement') {
            $field_value = get_post_meta($post_id, 'requirement', true );
            echo $field_value;
        }
        if ($column_name == 'party_role') {
            $field_value = get_post_meta($post_id, 'party_role', true );
            echo $field_value;
        }
        if ($column_name == 'name') {
