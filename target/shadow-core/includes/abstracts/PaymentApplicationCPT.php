...
class PaymentApplicationCPT {

    public static $prefix = ''; 

    public static $post_name = 'sb_payapp'; 
 ...
 public static $related_child_entities = array(
    ...
    {
       register_post_type('sb_payapp', 
            array(
                'label' => 'Payment Application',
                'labels' => array(
                'add_new'           => 'Add New',
                'add_new_item'      => 'Add New Payment Application',
                'edit_item'         => 'Edit Payment Application',
                'new_item'          => 'New Payment Application',
                'view_item'         => 'View Payment Application',
                'search_items'      => 'Search Payment Application',
                'not_found'         => 'No Payment Application Found ',
                'not_found_in_trash'=> 'Not Found in Trash',
                ),
                'description' => 'Reusable Payment Application',
                'public' => true,
                'show_ui' => true,
                'menu_position' => 5,
  ...
  ------------------------------------------------------------------------------*/
    ...
    public static function sb_payapp_table_head($defaults){
        $defaults['pa_payment']  = 'Payment';
        $defaults['pa_account']  = 'Billing Account';
        $defaults['pa_invoice']  = 'Invoice';
        $defaults['name']  = 'name';
    ...
    public static function sb_payapp_table_content($column_name, $post_id){
        if ($column_name == 'entity_code') {
            $field_value = get_post_meta($post_id, 'entity_code', true );
            echo $field_value;
        }
        if ($column_name == 'pa_payment') {
            $field_value = get_post_meta($post_id, 'pa_payment', true );
            echo $field_value;
        }
        if ($column_name == 'pa_account') {
            $field_value = get_post_meta($post_id, 'pa_account', true );
            echo $field_value;
        }
        if ($column_name == 'pa_invoice') {
            $field_value = get_post_meta($post_id, 'pa_invoice', true );
            echo $field_value;
        }
        if ($column_name == 'name') {
