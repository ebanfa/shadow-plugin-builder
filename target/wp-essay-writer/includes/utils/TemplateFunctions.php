...
function cp_get_template_part($slug, $name = '') {
    ...
    if ($name) {
        $template = locate_template(array("{$slug}-{$name}.php", WPEssayWriter::template_path() . "{$slug}-{$name}.php"));
    }
    // Get default slug-name.php
    if (!$template && $name && file_exists(WPEssayWriter::plugin_path() . "/templates/{$slug}-{$name}.php")) {
        $template = WPEssayWriter::plugin_path() . "/templates/{$slug}-{$name}.php";
    }
    // If template file doesn't exist, look in yourtheme/slug.php and yourtheme/woocommerce/slug.php
    if (!$template) {
        $template = locate_template(array("{$slug}.php", WPEssayWriter::template_path() . "{$slug}.php"));
    }
...
function cp_locate_template($template_name, $template_path = '', $default_path = '') {
    if (!$template_path) {
        $template_path = WPEssayWriter::template_path();
    }
    if (!$default_path) {
        $default_path = WPEssayWriter::plugin_path() . '/templates/';
    }
...
function locate_plugin_template($template_names, $load = false, $require_once = true )
{
    ...
    $this_plugin_dir = WPEssayWriter::plugin_path() . '/templates/';
