...
class ContentOrderAPI  {
    ...
    public static function do_create_entity($entity_data){
        ...
        if(!isset($party_data['id'])) { 
            ...
            if(isset($entity_data['email'])) {
                ...
                $party_data = CloderiaUserAPI::do_create_content_user($entity_data, true);
                // Sign the user in
    ...
    public static function prepare_signup_data($entity_data) {
        ...
        if (!empty($split_username)) {
            ...
            if(!isset($entity_data['last_name'])) $split_username[0];
            if(!isset($entity_data['first_name'])) $split_username[1];
        }
        // In case some how the above split failed and we dont have a first name and last name
