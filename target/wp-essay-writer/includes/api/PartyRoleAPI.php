...
class PartyRoleAPI {
    ...
    public static function has_role($party_id, $role_type_code){
        ...
        $party_roles = EntityAPI::find_by_criteria('partyrole', array('party_role_party' => $party_id, 'party_role_type' => $role_type_data['id']));
    ...
    public static function find_by_role($role) {
        ...
        if(isset($role_type['id'])) {
            ...
            $party_roles = EntityAPI::find_by_criteria('partyrole', array('party_role_type' => $role_type['id']));
            // Loop throug all the return party roles 
            // and push the party id into list of parties
            foreach ($party_roles as $party_role) {
                array_push($party_ids, $party_role['party_role_party']);
            }
            ...
            if(!empty($party_ids)) {
                ...
                foreach ($results as $key => $party_data) {
                    foreach ($party_roles as $party_role) {
                        if($party_data['id'] == $party_role['party_role_party'])
    ...
    public static function add_role_to_party($party_data, $role_type_code) {
        ...
        $role_type_data = EntityAPI::get_by_code('roletype', strtoupper($role_type_code));

        if(isset($role_type_data['id']) && isset($party_data['id'])) {
            $entity_data['edit_mode'] = true;
            $entity_data['name'] = $party_data['name'];
            $entity_data['party_role_party'] = $party_data['id'];
            $entity_data['party_role_type'] = $role_type_data['id'];
            $entity_data['description'] = 'Default role ' . $role_type_data['name'] . ' for party ' . $party_data['name'];
