...
class MailAPI {
    ...
    {   
        $context = self::get_order_data_context($content_order_data, $data_context);
     ...
     * 
     */
    public static function do_send_content_created_email($content_data, $data_context) 
    {   
        $context = self::get_content_data_context($content_data, $data_context);
        self::send_email($context, $context['user_name'], 'content-created-subject.tpl', 'content-created-message.tpl', array());
        self::send_email($context, get_option('cp_notify_orders'), 'content-created-subject.tpl', 'content-created-message.tpl', array());
    }
     ...
     * 
     */
    public static function do_send_contact_request_received_email($data_context)
    {   
        $data_context['user_name'] = $data_context['email'];
        $data_context['user_login'] = $data_context['email'];
        $data_context['user_pass'] = $data_context['email'];
        $data_context['first_name'] = $data_context['email'];
        $data_context['last_name'] = $data_context['email'];
        $context = self::get_email_data_context($data_context, array());
        self::send_email($context, get_option('cp_notify_accounts'), 'contactus-subject.tpl', 'contactus-message.tpl', array());;
    }
    ...
    public static function get_email_data_context($user_data, $additonal_context_data)
    {
        ...
        /*if(!isset($data_context['display_name'])) $data_context['display_name'] = $user_data['first_name'];
     ...
     * 
     */
    public static function get_content_data_context($entity_data, $additonal_context_data) {
        $content_user_data = UserPartyAPI::get_party_user($entity_data['content_party']);
        $data_context = self::get_email_data_context($content_user_data, $additonal_context_data);

        $data_context['content_title'] = $entity_data['name'];
        $data_context['content_text'] = $entity_data['description'];
        $data_context['content_subject'] = $entity_data['content_subject'];
        return $data_context;
    }
