    ...
    public function get_edit_url() {
        ...
        if(isset($party_type['id'])) {
            if($party_type['id'] == $this->model['party_type']) {
                $person_data = EntityAPI::get_by_field('person', 'person_party', $this->model['id']);
                if(isset($person_data['id'])) 
                    return ArtficatAjaxRequestProcessorUtils::get_base_url() . 'artifact=person&id=' . $person_data['id'] . $this->get_parent_url() . '&page_action=edit' . $role_param;
            }
            else {
                $partygroup_data = EntityAPI::get_by_field('partygroup', 'partygroup_party', $this->model['id']);
                if(isset($partygroup_data['id'])) 
                    return ArtficatAjaxRequestProcessorUtils::get_base_url() . 'artifact=partygroup&id=' . $partygroup_data['id'] . $this->get_parent_url() . '&page_action=edit' . $role_param;
            }
        }
        // execute default render operation
        return ArtficatAjaxRequestProcessorUtils::get_base_url() . 'artifact=' . $this->get_artifact_name() . '&id=' . $this->model['id'] . $this->get_parent_url() . '&page_action=edit' . $role_param;
    }
    ...
    public function get_delete_url() {
        ...
        if(isset($party_type['id'])) {
            if($party_type['id'] == $this->model['party_type']) {
                $person_data = EntityAPI::get_by_field('person', 'person_party', $this->model['id']);
                if(isset($person_data['id'])) 
                    return ArtficatAjaxRequestProcessorUtils::get_base_url() . 'artifact=person&id=' . $person_data['id'] . $this->get_parent_url() . '&page_action=delete' . $role_param;
            }
            else {
                $partygroup_data = EntityAPI::get_by_field('partygroup', 'partygroup_party', $this->model['id']);
                if(isset($partygroup_data['id'])) 
                    return ArtficatAjaxRequestProcessorUtils::get_base_url() . 'artifact=partygroup&id=' . $partygroup_data['id'] . $this->get_parent_url() . '&page_action=delete' . $role_param;
            }
        }
        // execute default render operation
        return ArtficatAjaxRequestProcessorUtils::get_base_url() . 'artifact=' . $this->get_artifact_name() . '&id=' . $this->model['id'] . $this->get_parent_url() . '&page_action=delete' . $role_param;
    }
