...
$(document).ready(function (){
   ...
   var sb_subareaimage_rows_selected = [];
   var sb_subareaimageTable =  $('#sb_subareaimage-multi-list-table').DataTable({
        "ajax": {
            'type': 'POST',
            'url': wpessaywriter_ajax_script.ajaxurl,
            'data': function(d){
               d.action = 'find_entity_ajax';
               d.form = $("#sb_subareaimage-multi-list-form").serializeArray();
            },
        },
        columns: [
            { data: "id" }, 

            { data: "file_subject_area_txt" },
            { data: "name" },
        ...
        columnDefs: [
            {
                ...
                'className': 'dt-body-center',
                'render': function (data, type, row){
                    return '<input id="subjectareaimage_' + row.id + '" type="checkbox" value="' + row.id + '" data-dependent-instance-name="' + row.name + '">';
                },
            },
            {
                // The `data` parameter refers to the data for the cell (defined by the
                ...
                "render": function ( data, type, row ) {
                    var parent_params = '';
                    if($('#subjectareaimage_parent_params').length) {
                        parent_params = parent_params + $('#subjectareaimage_parent_params').val(); 
                    }
                    return '<a class="data-table-link" href="' + wpessaywriter_base_url.baseUrl + 'artifact=subjectareaimage&id=' + row.id + '&page_action=view' + parent_params + '" data-related-artifact-name="subjectareaimage" data-related-instance-name="' + row.name + '" data-related-instance-id="' + row.id + '">' + data +  '</a>';
                },
                "targets": 1
            }
        ],
        ...
        'rowCallback': function(row, data, dataIndex){
         ...
         // If row ID is in the list of selected row IDs
         if($.inArray(rowId, sb_subareaimage_rows_selected) !== -1){
            $(row).find('input[type="checkbox"]').prop('checked', true);
            $(row).addClass('selected');
         }
        }
   ...
   // Handle click on checkbox
   $('#sb_subareaimage-multi-list-table tbody').on('click', 'input[type="checkbox"]', function(e){
      var $row = $(this).closest('tr');
      // Get row data
      var data = sb_subareaimageTable.row($row).data();
      // Get row ID
      var rowId = $(this).val();
      // Determine whether row ID is in the list of selected row IDs 
      var index = $.inArray(rowId, sb_subareaimage_rows_selected);
      console.log('This is index:' + index);
      // If checkbox is checked and row ID is not in list of selected row IDs
      if(this.checked && index === -1){
         console.log('This is checked and index:' + index);
         sb_subareaimage_rows_selected.push(rowId);
      // Otherwise, if checkbox is not checked and row ID is in list of selected row IDs
      } else if (!this.checked && index !== -1){
         sb_subareaimage_rows_selected.splice(index, 1);
         console.log('This is not checked and index:' + index);
      }
      if(this.checked){
         $row.css('background-color', 'rgba(255, 152, 0, 0.5)');
      ...
      // Update state of "Select all" control
      updateDataTableSelectAllCtrl(sb_subareaimageTable);
      // Prevent click event from propagating to parent
      e.stopPropagation();
   });

   // Handle click on table cells with checkboxes
   $('#sb_subareaimage-multi-list-table').on('click', 'tbody td, thead th:first-child', function(e){
      $(this).parent().find('input[type="checkbox"]').trigger('click');
   });

   // Handle click on "Select all" control
   $('thead input[name="select_all"]', sb_subareaimageTable.table().container()).on('click', function(e){
      if(this.checked){
         $('#sb_subareaimage-multi-list-table tbody input[type="checkbox"]:not(:checked)').trigger('click');
      } else {
         $('#sb_subareaimage-multi-list-table tbody input[type="checkbox"]:checked').trigger('click');
      }
      // Prevent click event from propagating to parent
      e.stopPropagation();
   });
   ...
   // Handle table draw event
   sb_subareaimageTable.on('draw', function(){
      // Update state of "Select all" control
      updateDataTableSelectAllCtrl(sb_subareaimageTable);
   });

   /* Add all check rows in the data table */
   $('body').on('click', '#add-selected-subjectareaimage-list-btn', function(e){
      e.preventDefault();
      var page_artifact_form = $('#main-entity-post-name').val();
      // Iterate over all selected checkboxes
      var idExists = false;
      $.each(sb_subareaimage_rows_selected, function(index, rowId){

        $.each($('input[name="subjectareaimage_id[]"]'), function(indexx){ 
            var valueToAdd = $(this).val();
            if(valueToAdd === rowId){
              idExists = true;
            }
        ...
        if(!idExists){
          ...
          // main form 
           $('#' + page_artifact_form).append(
               $('<input>')
                  .attr('type', 'hidden')
                  .attr('name', 'subjectareaimage_id[]')
                  .val(rowId)
           );
           // Get the value of the name column. Every entity data table has name and description columns
           var dependentInstanceName = $('#subjectareaimage_' + rowId).data('dependent-instance-name');
           // Add an entry into the visual list of select instances
           $('#subjectareaimage_dependent_list_box').append($(
                '<div id="subjectareaimage_list_item_' + rowId + '"> ' + 
                    '<span data-entity-name="subjectareaimage" ' + 
                        'data-entity-id="' + rowId + '" class="badge subjectareaimage_dependent_list_item" ' + 
                        'style="cursor: pointer; cursor: hand; background-color: red">X</span>' + dependentInstanceName + 
                '</div>').attr('class', 'list-group-item'));
        }
      });
    ...
    // Handle click on table cells with checkboxes
   $('#subjectareaimage_dependent_list_box').on('click', '.subjectareaimage_dependent_list_item', function(e){
      var entityId = $(this).data('entity-id');
      var entityName = $(this).data('entity-name');
      var page_artifact_form = $('#main-entity-post-name').val();
      // first remove the hidden form field and then the list box item
      var existingIds = $('#' + page_artifact_form).find('input[name="subjectareaimage_id[]"]');
      $.each(existingIds, function(index, rowId){ 
        if($(rowId).val() == entityId) {
          $(rowId).remove();
        }
      ...
      // then remove the list box item
      $('#subjectareaimage_dependent_list_box').find('#subjectareaimage_list_item_' + entityId).remove();
   ...
   var sb_partyeducation_rows_selected = [];
   var sb_partyeducationTable =  $('#sb_partyeducation-multi-list-table').DataTable({
        "ajax": {
            'type': 'POST',
            'url': wpessaywriter_ajax_script.ajaxurl,
            'data': function(d){
               d.action = 'find_entity_ajax';
               d.form = $("#sb_partyeducation-multi-list-form").serializeArray();
            },
        },
        columns: [
            { data: "id" }, 
        ...
        columnDefs: [
            {
                ...
                'className': 'dt-body-center',
                'render': function (data, type, row){
                    return '<input id="partyeducation_' + row.id + '" type="checkbox" value="' + row.id + '" data-dependent-instance-name="' + row.name + '">';
                },
            },
            {
                // The `data` parameter refers to the data for the cell (defined by the
                ...
                "render": function ( data, type, row ) {
                    var parent_params = '';
                    if($('#partyeducation_parent_params').length) {
                        parent_params = parent_params + $('#partyeducation_parent_params').val(); 
                    }
                    return '<a class="data-table-link" href="' + wpessaywriter_base_url.baseUrl + 'artifact=partyeducation&id=' + row.id + '&page_action=view' + parent_params + '" data-related-artifact-name="partyeducation" data-related-instance-name="' + row.name + '" data-related-instance-id="' + row.id + '">' + data +  '</a>';
                },
                "targets": 1
            }
        ],
        ...
        'rowCallback': function(row, data, dataIndex){
         ...
         // If row ID is in the list of selected row IDs
         if($.inArray(rowId, sb_partyeducation_rows_selected) !== -1){
            $(row).find('input[type="checkbox"]').prop('checked', true);
            $(row).addClass('selected');
         }
        }
   ...
   // Handle click on checkbox
   $('#sb_partyeducation-multi-list-table tbody').on('click', 'input[type="checkbox"]', function(e){
      var $row = $(this).closest('tr');
      // Get row data
      var data = sb_partyeducationTable.row($row).data();
      // Get row ID
      var rowId = $(this).val();
      // Determine whether row ID is in the list of selected row IDs 
      var index = $.inArray(rowId, sb_partyeducation_rows_selected);
      console.log('This is index:' + index);
      // If checkbox is checked and row ID is not in list of selected row IDs
      if(this.checked && index === -1){
         console.log('This is checked and index:' + index);
         sb_partyeducation_rows_selected.push(rowId);
      // Otherwise, if checkbox is not checked and row ID is in list of selected row IDs
      } else if (!this.checked && index !== -1){
         sb_partyeducation_rows_selected.splice(index, 1);
         console.log('This is not checked and index:' + index);
      }
      if(this.checked){
         $row.css('background-color', 'rgba(255, 152, 0, 0.5)');
      ...
      // Update state of "Select all" control
      updateDataTableSelectAllCtrl(sb_partyeducationTable);
      // Prevent click event from propagating to parent
      e.stopPropagation();
   });

   // Handle click on table cells with checkboxes
   $('#sb_partyeducation-multi-list-table').on('click', 'tbody td, thead th:first-child', function(e){
      $(this).parent().find('input[type="checkbox"]').trigger('click');
   });

   // Handle click on "Select all" control
   $('thead input[name="select_all"]', sb_partyeducationTable.table().container()).on('click', function(e){
      if(this.checked){
         $('#sb_partyeducation-multi-list-table tbody input[type="checkbox"]:not(:checked)').trigger('click');
      } else {
         $('#sb_partyeducation-multi-list-table tbody input[type="checkbox"]:checked').trigger('click');
      }
      // Prevent click event from propagating to parent
      e.stopPropagation();
   });
   ...
   // Handle table draw event
   sb_partyeducationTable.on('draw', function(){
      // Update state of "Select all" control
      updateDataTableSelectAllCtrl(sb_partyeducationTable);
   });

   /* Add all check rows in the data table */
   $('body').on('click', '#add-selected-partyeducation-list-btn', function(e){
      e.preventDefault();
      var page_artifact_form = $('#main-entity-post-name').val();
      // Iterate over all selected checkboxes
      var idExists = false;
      $.each(sb_partyeducation_rows_selected, function(index, rowId){

        $.each($('input[name="partyeducation_id[]"]'), function(indexx){ 
            var valueToAdd = $(this).val();
            if(valueToAdd === rowId){
              idExists = true;
            }
        ...
        if(!idExists){
          ...
          // main form 
           $('#' + page_artifact_form).append(
               $('<input>')
                  .attr('type', 'hidden')
                  .attr('name', 'partyeducation_id[]')
                  .val(rowId)
           );
           // Get the value of the name column. Every entity data table has name and description columns
           var dependentInstanceName = $('#partyeducation_' + rowId).data('dependent-instance-name');
           // Add an entry into the visual list of select instances
           $('#partyeducation_dependent_list_box').append($(
                '<div id="partyeducation_list_item_' + rowId + '"> ' + 
                    '<span data-entity-name="partyeducation" ' + 
                        'data-entity-id="' + rowId + '" class="badge partyeducation_dependent_list_item" ' + 
                        'style="cursor: pointer; cursor: hand; background-color: red">X</span>' + dependentInstanceName + 
                '</div>').attr('class', 'list-group-item'));
        }
      });
    ...
    // Handle click on table cells with checkboxes
   $('#partyeducation_dependent_list_box').on('click', '.partyeducation_dependent_list_item', function(e){
      var entityId = $(this).data('entity-id');
      var entityName = $(this).data('entity-name');
      var page_artifact_form = $('#main-entity-post-name').val();
      // first remove the hidden form field and then the list box item
      var existingIds = $('#' + page_artifact_form).find('input[name="partyeducation_id[]"]');
      $.each(existingIds, function(index, rowId){ 
        if($(rowId).val() == entityId) {
          $(rowId).remove();
        }
      ...
      // then remove the list box item
      $('#partyeducation_dependent_list_box').find('#partyeducation_list_item_' + entityId).remove();

      
   });
   var sb_partysubject_rows_selected = [];
   var sb_partysubjectTable =  $('#sb_partysubject-multi-list-table').DataTable({
        "ajax": {
            'type': 'POST',
            'url': wpessaywriter_ajax_script.ajaxurl,
            'data': function(d){
               d.action = 'find_entity_ajax';
               d.form = $("#sb_partysubject-multi-list-form").serializeArray();
            },
        },
        columns: [
            { data: "id" }, 
        ...
        columnDefs: [
            {
                ...
                'className': 'dt-body-center',
                'render': function (data, type, row){
                    return '<input id="partysubject_' + row.id + '" type="checkbox" value="' + row.id + '" data-dependent-instance-name="' + row.name + '">';
                },
            },
            {
                // The `data` parameter refers to the data for the cell (defined by the
                ...
                "render": function ( data, type, row ) {
                    var parent_params = '';
                    if($('#partysubject_parent_params').length) {
                        parent_params = parent_params + $('#partysubject_parent_params').val(); 
                    }
                    return '<a class="data-table-link" href="' + wpessaywriter_base_url.baseUrl + 'artifact=partysubject&id=' + row.id + '&page_action=view' + parent_params + '" data-related-artifact-name="partysubject" data-related-instance-name="' + row.name + '" data-related-instance-id="' + row.id + '">' + data +  '</a>';
                },
                "targets": 1
            }
        ],
        ...
        'rowCallback': function(row, data, dataIndex){
         ...
         // If row ID is in the list of selected row IDs
         if($.inArray(rowId, sb_partysubject_rows_selected) !== -1){
            $(row).find('input[type="checkbox"]').prop('checked', true);
            $(row).addClass('selected');
         }
        }
   ...
   // Handle click on checkbox
   $('#sb_partysubject-multi-list-table tbody').on('click', 'input[type="checkbox"]', function(e){
      var $row = $(this).closest('tr');
      // Get row data
      var data = sb_partysubjectTable.row($row).data();
      // Get row ID
      var rowId = $(this).val();
      // Determine whether row ID is in the list of selected row IDs 
      var index = $.inArray(rowId, sb_partysubject_rows_selected);
      console.log('This is index:' + index);
      // If checkbox is checked and row ID is not in list of selected row IDs
      if(this.checked && index === -1){
         console.log('This is checked and index:' + index);
         sb_partysubject_rows_selected.push(rowId);
      // Otherwise, if checkbox is not checked and row ID is in list of selected row IDs
      } else if (!this.checked && index !== -1){
         sb_partysubject_rows_selected.splice(index, 1);
         console.log('This is not checked and index:' + index);
      }
      if(this.checked){
         $row.css('background-color', 'rgba(255, 152, 0, 0.5)');
      ...
      // Update state of "Select all" control
      updateDataTableSelectAllCtrl(sb_partysubjectTable);
      // Prevent click event from propagating to parent
      e.stopPropagation();
   });

   // Handle click on table cells with checkboxes
   $('#sb_partysubject-multi-list-table').on('click', 'tbody td, thead th:first-child', function(e){
      $(this).parent().find('input[type="checkbox"]').trigger('click');
   });

   // Handle click on "Select all" control
   $('thead input[name="select_all"]', sb_partysubjectTable.table().container()).on('click', function(e){
      if(this.checked){
         $('#sb_partysubject-multi-list-table tbody input[type="checkbox"]:not(:checked)').trigger('click');
      } else {
         $('#sb_partysubject-multi-list-table tbody input[type="checkbox"]:checked').trigger('click');
      }
      // Prevent click event from propagating to parent
      e.stopPropagation();
   });
   ...
   // Handle table draw event
   sb_partysubjectTable.on('draw', function(){
      // Update state of "Select all" control
      updateDataTableSelectAllCtrl(sb_partysubjectTable);
   });

   /* Add all check rows in the data table */
   $('body').on('click', '#add-selected-partysubject-list-btn', function(e){
      e.preventDefault();
      var page_artifact_form = $('#main-entity-post-name').val();
      // Iterate over all selected checkboxes
      var idExists = false;
      $.each(sb_partysubject_rows_selected, function(index, rowId){

        $.each($('input[name="partysubject_id[]"]'), function(indexx){ 
            var valueToAdd = $(this).val();
            if(valueToAdd === rowId){
              idExists = true;
            }
        ...
        if(!idExists){
          ...
          // main form 
           $('#' + page_artifact_form).append(
               $('<input>')
                  .attr('type', 'hidden')
                  .attr('name', 'partysubject_id[]')
                  .val(rowId)
           );
           // Get the value of the name column. Every entity data table has name and description columns
           var dependentInstanceName = $('#partysubject_' + rowId).data('dependent-instance-name');
           // Add an entry into the visual list of select instances
           $('#partysubject_dependent_list_box').append($(
                '<div id="partysubject_list_item_' + rowId + '"> ' + 
                    '<span data-entity-name="partysubject" ' + 
                        'data-entity-id="' + rowId + '" class="badge partysubject_dependent_list_item" ' + 
                        'style="cursor: pointer; cursor: hand; background-color: red">X</span>' + dependentInstanceName + 
                '</div>').attr('class', 'list-group-item'));
        }
      });
    ...
    // Handle click on table cells with checkboxes
   $('#partysubject_dependent_list_box').on('click', '.partysubject_dependent_list_item', function(e){
      var entityId = $(this).data('entity-id');
      var entityName = $(this).data('entity-name');
      var page_artifact_form = $('#main-entity-post-name').val();
      // first remove the hidden form field and then the list box item
      var existingIds = $('#' + page_artifact_form).find('input[name="partysubject_id[]"]');
      $.each(existingIds, function(index, rowId){ 
        if($(rowId).val() == entityId) {
          $(rowId).remove();
        }
      ...
      // then remove the list box item
      $('#partysubject_dependent_list_box').find('#partysubject_list_item_' + entityId).remove();
   ...
   var sb_contentTable =  $('#sb_content-multi-list-table').DataTable({
        ...
        columns: [
            ...
            { data: "content_party_txt" },

            { data: "content_tutor_txt" },
